import { MessageState } from "@/plugins/interface";
import firebase from "firebase";

// https://firebase.google.com/docs/reference/js/firebase.auth.Error#code
export const handleFirebaseAuthError = (error: firebase.auth.Error): MessageState => {
  console.error(error);
  switch (error.code) {
    case 'auth/app-deleted':
      // Thrown if the instance of FirebaseApp has been deleted.
      return { type: 'error', text: 'アプリケーションが存在しません。' };
    ...
    case 'auth/operation-not-allowed':
      ...
      return { type: 'error', text: '許可されていない操作です。' };
    ...
    case 'auth/unauthorized-domain':
      ...
      return { type: 'error', text: '許可されていないドメインです。' };
    case 'auth/user-disabled':
      ...
      return { type: 'error', text: '指定されたユーザーは使用できません。' };
    case 'auth/user-token-expired':
      ...
      return { type: 'error', text: 'ユーザーのトークンが期限切れです。' };
    case 'auth/web-storage-unsupported':
      return { type: 'error', text: 'ストレージが使用できません' };
    case 'auth/provider-already-linked':
      return { type: 'error', text: '既に連携済みのアカウントです。' };
    case 'auth/no-such-provider':
      return { type: 'error', text: '未連携のアカウントです。' };
  }
  return { type: 'error', text: 'エラーが発生しました' };
};

// https://firebase.google.com/docs/reference/js/firebase.firestore#firestoreerrorcode
export const handleFirestoreError = (error: firebase.firestore.FirestoreError): MessageState => {
  console.log(error);
  switch (error.code) {
    case 'cancelled':
      //  The operation was cancelled (typically by the caller).
      return { type: 'error', text: '処理がキャンセルされました。' };
    case 'unknown':
    ...
    case 'invalid-argument':
      ...
      return { type: 'error', text: 'パラメーターに問題があります。' };
    case 'deadline-exceeded':
      //  Deadline expired before operation could complete. For operations that change the state of the system, this error may be returned even if the operation has completed successfully. For example, a successful response from a server could have been delayed long enough for the deadline to expire.
      return { type: 'error', text: '処理制限時間を超過しました。' };
    case 'not-found':
      //  Some requested document was not found.
      return { type: 'error', text: 'データがありません。' };
    case 'already-exists':
      //  Some document that we attempted to create already exists.
      return { type: 'error', text: '既にデータが存在します。' };
    case 'permission-denied':
      //  The caller does not have permission to execute the specified operation.
      return { type: 'error', text: '権限がありません。' };
    case 'resource-exhausted':
      //  Some resource has been exhausted, perhaps a per-user quota, or perhaps the entire file system is out of space.
      return { type: 'error', text: '処理に必要なリソースが足りません。' };
    case 'failed-precondition':
      //  Operation was rejected because the system is not in a state required for the operation's execution.
      return { type: 'error', text: '処理を実行できませんでした。' };
    case 'aborted':
      //  The operation was aborted, typically due to a concurrency issue like transaction aborts, etc.
      return { type: 'error', text: '処理が中断されました。' };
    case 'out-of-range':
      //  Operation was attempted past the valid range.
      return { type: 'error', text: '範囲外です。' };
    case 'unimplemented':
      //  Operation is not implemented or not supported/enabled.
      return { type: 'error', text: '使用できない操作です。' };
    case 'internal':
      //  Internal errors. Means some invariants expected by underlying system has been broken. If you see one of these errors, something is very broken.
      return { type: 'error', text: 'システムエラーが発生しました。' };
    case 'unavailable':
      ...
      return { type: 'error', text: '現在サービスが利用できません。しばらくしてから再度実行してください。' };
    case 'data-loss':
      //  Unrecoverable data loss or corruption.
      return { type: 'error', text: 'データが欠損しました。' };
    case 'unauthenticated':
      //  The request does not have valid authentication credentials for the operation.
      return { type: 'error', text: '認証に失敗しました。' };
  }
  return { type: 'error', text: 'エラーが発生しました。' };
};

// https://firebase.google.com/docs/reference/js/firebase.storage.FirebaseStorageError
export const handleFirebaseStorageError = (error: firebase.storage.FirebaseStorageError): MessageState => {
  console.log(error);
  return { type: 'error', text: 'エラーが発生しました。' };
};